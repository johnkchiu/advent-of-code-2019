...
func createGrid(path []string) *Grid {
	grid := NewGrid()
	x, y := 0, 0
	for _, move := range path {
		x, y = updateGrid(grid, move, x, y)
	}
	return grid
}

func updateGrid(grid *Grid, move string, x int, y int) (int, int) {
	// fmt.Printf("Starting[%v,%v] Moving[%v]\n", x, y, move)
	dist, _ := strconv.Atoi(move[1:])
	switch move[0] {
	case 'R':
		// fmt.Printf("Right %v\n", dist)
		for i := x; i <= x+dist; i++ {
			grid.Set(Point{i, y}, true)
		}
		return x + dist, y
	case 'L':
		// fmt.Printf("Left %v\n", dist)
	...
	case 'U':
		// fmt.Printf("Up %v\n", dist)
		for i := y; i <= y+dist; i++ {
			grid.Set(Point{x, i}, true)
		}
		return x, y + dist
	case 'D':
		// fmt.Printf("Down %v\n", dist)
		for i := y; i >= y-dist; i-- {
			// fmt.Printf("Down x[%v] y[%v] i[%v] y+dist[%v]", x, y, i, y+dist)
			grid.Set(Point{x, i}, true)
		}
		return x, y - dist
	}
